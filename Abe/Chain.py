...
def create(policy, **kwargs):
    ...
    if policy == "YellowCoin":      return YellowCoin(**kwargs)
...
class YellowCoin(NvcChain):
    def __init__(chain, **kwargs):
        chain.name = 'YellowCoin'
        chain.code3 = 'YC'
        chain.address_version = "\x4d"
        chain.magic = "\xce\xfb\xfa\xdb"     
        # chain.decimals = 6
        Chain.__init__(chain, **kwargs)

    datadir_conf_file_name = "YellowCoin.conf"
    datadir_rpcport = 16889    
